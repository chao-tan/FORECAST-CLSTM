...
from network_scmd import FORECAST_CLSTM_S,CLSTM_S,MLP_S,FC_LSTM_S
from utils import Bar, Logger, AverageMeter
import dataset
from tools import FORECASTER_LOSS


parser = argparse.ArgumentParser(description='CLSTM Training')

parser.add_argument("--model",type=str,default="FORECAST_CLSTM_S",help="model name,optional in (FORECAST_CLSTM_S,CLSTM_S,MLP_S,FC_LSTM_S)")
...
parser.add_argument('--train-iters',default="10000",type=int,help="training iterations per epoch")
parser.add_argument('--test-iters',default="2000",type=int,help="test iterations per epoch")
...
parser.add_argument('--checkpoint', default="checkpoint/forecast_clstm_s", type=str, metavar='PATH')
...
if args.model == "FORECAST_CLSTM_S":
    net = FORECAST_CLSTM_S
elif args.model == 'CLSTM_S':
    net = CLSTM_S
elif args.model == 'MLP_S':
    net = MLP_S
elif args.model == 'FC_LSTM_S':
    net = FC_LSTM_S
else:
    pass
...
scmd_train = dataset.SCMD_Generation(norm=False,isTrain=True)
scmd_test = dataset.SCMD_Generation(norm=False,isTrain=False)
...
def train(model, criterion, optimizer):
    ...
    for i in range(int(args.train_iters/args.train_batch)):
        data_time.update(time.time() - end)

        inputs,targets = scmd_train.next_batch(batchsize=args.train_batch)
...
def test(model, criterion):
    ...
    for i in range(int(args.test_iters/args.test_batch)):
        ...
        inputs, targets = scmd_test.next_batch(batchsize=args.train_batch)
        inputs = torch.from_numpy(inputs).float()
        targets = torch.from_numpy(targets).float()
